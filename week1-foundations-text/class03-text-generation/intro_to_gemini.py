...
def search_wikipedia(query):
    ...
    if response.status_code == 200:
        ...
        print(f"Invalid search query. No articles found. Status code: {response.status_code}")
...
def integrate_api_calls_with_gemini(model:str="gemini-2.5-flash", max_iterations:int=5):
    ...
    while not answer_found:
        ...
        if count == 10:
            messages.append({
                "role": "user", 
                "content":f"This is your last chance to find the answer to the user's question. Please reflect on your previous attempts: {generate_reflection(list(messages))}"
            })
        ...
        if response.choices[0].message.tool_calls:
            ...
            for tool_call in tool_calls:
                ...
                if tool_call.function.name == "encyclopedia_search":
                    ...
                    print("Took ", count, "iterations to find the answer")
                    answer_found = True
                    break
